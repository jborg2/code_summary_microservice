...
class CallGraphVisitor(ast.NodeVisitor):
            ...
            if module_uses is not None:
                ...
                for candidate_to_node in module_uses:
                    if candidate_to_node.name == from_node.name:
                        ...
                        break
            print(f"Import {from_node} -> {to_node}, {len(imports_to_resolve)} remaining")
    ...
    def visit_Constant(self, node):
        ...
        t = type(node.value)
        if self.get_node_of_current_namespace() is None:
            return
    ...
    def contract_nonexistents(self):
        ...
        new_uses_edges = []
        removed_uses_edges = []
        print("contacting non-existant")
        for n in tqdm.tqdm(self.uses_edges):
    ...
    def expand_unknowns(self):
        ...
        for n in tqdm.tqdm(self.defines_edges):
        ...
        for from_node, to_node in tqdm.tqdm(new_uses_edges):
            self.add_uses_edge(from_node, to_node)
            self.logger.info("Expanding unknowns: new uses edge from %s to %s" % (from_node, to_node))
        print("Marking unknowns as not defined...")
        for name in tqdm.tqdm(self.nodes):
            for n in self.nodes[name]:
    ...
    def collapse_inner(self):
        ...
        print("Collapsing inner...")
        for name in tqdm.tqdm(list(self.nodes)):
