...
class VisualNode(object):
    ...
    def __init__(self, id, label="", flavor="", fill_color="", text_color="", group="", node=None):
...
class VisualGraph(object):
    ...
    def from_visitor(cls, visitor, options=None, logger=None):
        ...
        for edge in root_graph.edges:
            source = edge[0]
            target = edge[1]
            root_graph.nodes[source]["subcalls"].append(target)
        file_map = {}
        for node in root_graph.nodes:
            if node.filename not in file_map:
